...
function sentences(content: string, queryTerms: Set<string>) {
  return content.replace(/\s+/g, ' ').split(/(?<=[.!?])\s+/).map(text => text.trim()).filter(text => text.length >= 25).map(text => ({ text, score: [...queryTerms].reduce((total, term) => total + (text.toLowerCase().includes(term) ? 1 : 0), 0) })).filter(item => item.score > 0).sort((a, b) => b.score - a.score || a.text.length - b.text.length).slice(0, 2).map(item => item.text);
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const saved = [...allowedDocumentIds].map(id => db.getDocumentById(id)).filter(Boolean) as any[];
    ...
    const ranked = docs.flatMap(doc => doc.chunks.map((chunk: any) => {
      const keywordScore = [...queryTerms].filter(term => chunk.content.toLowerCase().includes(term)).length / queryTerms.size;
    ...
    const selected = selectedIds.length ? [...new Set(selectedIds)].map(id => evidence[id]) : evidence.slice(0, 2);
